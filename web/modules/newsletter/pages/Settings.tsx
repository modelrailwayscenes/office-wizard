    ...
    <div className="p-8 max-w-4xl mx-auto animate-fade-in">
      ...
      <Tabs defaultValue="company" className="space-y-6">
        <TabsList className="grid w-full grid-cols-5">
          ...
          <TabsTrigger value="brand" className="gap-1.5 text-xs"><Palette className="w-3.5 h-3.5" /> Brand</TabsTrigger>
          <TabsTrigger value="shopify" className="gap-1.5 text-xs"><Globe className="w-3.5 h-3.5" /> Shopify</TabsTrigger>
          <TabsTrigger value="output" className="gap-1.5 text-xs"><Share2 className="w-3.5 h-3.5" /> Output</TabsTrigger>
        </TabsList>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2 mt-1.5">
                  <Input value={settings.company.logoUrl} onChange={e => update('company', { logoUrl: e.target.value })} placeholder="https://..." />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Company Name</Label><Input value={settings.company.name} onChange={e => update('company', { name: e.target.value })} className="mt-1.5" /></div>
                <div><Label>Tagline</Label><Input value={settings.company.tagline} onChange={e => update('company', { tagline: e.target.value })} className="mt-1.5" /></div>
              </div>
              <div><Label>Address</Label><Textarea value={settings.company.address} onChange={e => update('company', { address: e.target.value })} className="mt-1.5" rows={2} placeholder="123 Railway Lane, UK" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Email</Label><Input value={settings.company.email} onChange={e => update('company', { email: e.target.value })} className="mt-1.5" placeholder="hello@..." /></div>
                <div><Label>Phone</Label><Input value={settings.company.phone} onChange={e => update('company', { phone: e.target.value })} className="mt-1.5" /></div>
              </div>
              <div><Label>Website</Label><Input value={settings.company.website} onChange={e => update('company', { website: e.target.value })} className="mt-1.5" /></div>
            </div>

            <Separator />
            <h3 className="text-base font-semibold">Social Media Links</h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2"><Instagram className="w-4 h-4 text-muted-foreground" /><Input value={settings.social.instagram} onChange={e => update('social', { instagram: e.target.value })} placeholder="Instagram URL" /></div>
              <div className="flex items-center gap-2"><Facebook className="w-4 h-4 text-muted-foreground" /><Input value={settings.social.facebook} onChange={e => update('social', { facebook: e.target.value })} placeholder="Facebook URL" /></div>
              <div className="flex items-center gap-2"><Youtube className="w-4 h-4 text-muted-foreground" /><Input value={settings.social.youtube} onChange={e => update('social', { youtube: e.target.value })} placeholder="YouTube URL" /></div>
              <div className="flex items-center gap-2"><Twitter className="w-4 h-4 text-muted-foreground" /><Input value={settings.social.twitter} onChange={e => update('social', { twitter: e.target.value })} placeholder="X / Twitter URL" /></div>
            </div>
          </Card>
        </TabsContent>
          ...
          <Card className="p-6 space-y-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input value={settings.email.subjectPrefix} onChange={e => update('email', { subjectPrefix: e.target.value })} placeholder="e.g. [MRS] or 🚂 MRS:" className="mt-1.5" />
                <p className="text-xs text-muted-foreground mt-1">This text is automatically prepended to every newsletter subject line</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><Label>From Name</Label><Input value={settings.email.fromName} onChange={e => update('email', { fromName: e.target.value })} className="mt-1.5" /></div>
                <div><Label>Reply-To Email</Label><Input value={settings.email.replyTo} onChange={e => update('email', { replyTo: e.target.value })} className="mt-1.5" placeholder="hello@..." /></div>
              </div>
              <div>
                <Label>Default Footer Text</Label>
                <Textarea value={settings.email.defaultFooterText} onChange={e => update('email', { defaultFooterText: e.target.value })} className="mt-1.5" rows={3} placeholder="You're receiving this because you signed up at modelrailwayscenes.com..." />
              </div>
            </div>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="brand">
          <Card className="p-6 space-y-5">
            <h2 className="text-lg font-semibold">Brand Colour Presets</h2>
            ...
            {editingBrand ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold">{BRAND_PROFILES.find(b => b.id === editingBrand)?.name} — Edit Colours</h3>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={() => setEditingBrand(null)}>Cancel</Button>
                    <Button size="sm" onClick={saveBrandOverride}>Save Colours</Button>
                  </div>
                </div>
                  ...
                  {Object.entries(brandColors).map(([key, val]) => (
                    <div key={key}>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <Input value={val} onChange={e => setBrandColors(c => ({ ...c, [key]: e.target.value }))} className="font-mono text-xs h-8" />
                      </div>
                    </div>
                  ))}
                </div>
                <div className="mt-4">
                  ...
                  <div className="mt-2 rounded-lg overflow-hidden border" style={{ maxWidth: '400px' }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {BRAND_PROFILES.map(profile => {
                  ...
                  return (
                    <Card key={profile.id} className="overflow-hidden cursor-pointer hover:shadow-card transition-shadow" onClick={() => startEditBrand(profile.id)}>
                      ...
                      <div className="p-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {hasOverride && <Badge variant="outline" className="text-xs">Customised</Badge>}
          ...
          <Card className="p-6 space-y-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input value={settings.shopify.storeUrl} onChange={e => update('shopify', { storeUrl: e.target.value })} placeholder="your-store.myshopify.com" className="mt-1.5" />
              </div>
              ...
              <div className="border rounded-lg p-4 space-y-3">
                <h3 className="text-sm font-semibold">When connected, you can:</h3>
        ...
        <TabsContent value="output">
          <Card className="p-6 space-y-5">
            <h2 className="text-lg font-semibold">Output & Export</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-2">
                  {[
                    ...
                    <Card
                      key={opt.id}
                      className={`p-4 cursor-pointer transition-all ${settings.output.defaultFormat === opt.id ? 'ring-2 ring-primary' : 'hover:shadow-card'}`}
              ...
              <div>
                <Label>Email Max Width (px)</Label>
                <Input type="number" value={settings.output.emailWidth} onChange={e => update('output', { emailWidth: parseInt(e.target.value) || 600 })} className="mt-1.5 max-w-32" />
